...
class JobOpen(db.Model): # 구인게시글
    ...
    update_date = db.Column(db.DateTime(), nullable=False)
    user_name = db.Column(db.String(150), db.ForeignKey('user.username', ondelete='CASCADE'), nullable=False)
    user = db.relationship("User", backref=db.backref("job_open_set", cascade='all, delete-orphan'))
...
class JobHunt(db.Model): # 구직게시글
    ...
    update_date = db.Column(db.DateTime(), nullable=False)
    user_name = db.Column(db.String(150), db.ForeignKey('user.username', ondelete='CASCADE'), nullable=False)
    user = db.relationship("User", backref=db.backref("job_hunt_set", cascade='all, delete-orphan'))